...
CMAP = ['#000000', '#E69F00', '#56B4E9', '#009E73', '#F0E442', '#0072B2', '#D55E00', '#CC79A7', '#FFFFFF']  # Wong
...
def plotter(path, include_mpc_u=False):
    ...
    for skill in range(n_skills):
        ...
        # Input
        if include_mpc_u:
            fig.add_trace(go.Scatter(x=_t, y=(h[:, skill]) + 1, line=dict(shape='hv', color=CMAP[-skill-1]),
                                     name="Question inputs",
                                     showlegend=False), row=3, col=1)
            fig.add_trace(go.Scatter(x=_t, y=(h_optimal[:, skill]), line=dict(shape='hv', color=CMAP[-skill-1]),
                                     name="MPC optimal question",
                                     yaxis='y5',
                                     xaxis='x3',
                                     showlegend=False))#, row=3, col=1)
        else:
            fig.add_trace(go.Scatter(x=_t, y=h[:, skill], line=dict(shape='hv', color=CMAP[-skill - 1]),
                                     name="Question inputs",
                                     showlegend=False), row=3, col=1)
...
if __name__ == '__main__':
    path_original = 'src/simulations/mpc_mhe_model/student_4.pkl'
    path_random = 'simulations/benchmark_random/student_4.pkl'
    path_increasing = 'simulations/benchmark_increasing_difficulty/student_4.pkl'

    fig = plotter(path_original, include_mpc_u=True)
    fig.write_image(path_original[:-4]+'.png')
    '''
    # Load data:
